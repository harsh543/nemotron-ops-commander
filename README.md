...
- 30 real-world incidents covering Kubernetes, cloud, networking, CI/CD,
  and application failures.
- Embedded ChromaDB vector store (no external DB required).
- sentence-transformers embeddings (all-MiniLM-L6-v2).

## Model usage

- Primary: NVIDIA Nemotron-Mini-4B-Instruct.
- Demo: Hugging Face Space for easy sharing.
- Production: local GPU via SGLang for lower latency and batching.
...
Local run steps are documented in [DEMO_GUIDE.md](DEMO_GUIDE.md).

## Demo flow (for judges)

1. Knowledge Search — instant RAG results
2. Log Analysis — severity + root cause + recommendations
3. Incident Triage — P0–P4 priority + next steps
4. Performance Optimizer — metric‑driven remediation

## Security and observability

- API key enforcement via X-API-Key
...
**AI-Powered Incident Response System using NVIDIA Nemotron**

An intelligent operations assistant that helps SRE teams analyze logs, triage incidents, and automate remediation — powered by NVIDIA Nemotron with SGLang optimizations. Runs completely locally with no external API dependencies for core functionality.
